...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('DROP TABLE IF EXISTS bandhan.validation_lot_details CASCADE');

        DB::statement("
            CREATE TABLE IF NOT EXISTS bandhan.validation_lot_details (
                ...
                CONSTRAINT validation_lot_details_pkey PRIMARY KEY (scheme_id, id)
            )
            PARTITION BY LIST (scheme_id);
        ");

        DB::statement("
            ALTER TABLE bandhan.validation_lot_details 
            ADD CONSTRAINT fk_validation_lot_details_av_account_status FOREIGN KEY (av_account_status) 
            REFERENCES bandhan.codemasters(code)
        ");

        DB::statement("
            ALTER TABLE bandhan.validation_lot_details 
            ADD CONSTRAINT fk_validation_lot_details_ben_id FOREIGN KEY (ben_id,scheme_id) 
            REFERENCES payment.ben_payment_details(ben_id,scheme_id)
        ");

        DB::statement("
            ALTER TABLE bandhan.validation_lot_details 
            ADD CONSTRAINT fk_validation_lot_details_lot_scheme FOREIGN KEY (lot_no,scheme_id) 
            REFERENCES payment.validation_lot_master(lot_no,scheme_id)
        ");

        DB::statement("
            ALTER TABLE bandhan.validation_lot_details 
            ADD CONSTRAINT fk_validation_lot_details_name_status_code FOREIGN KEY (name_status_code) 
            REFERENCES bandhan.codemasters(code)
        ");
        ...
        foreach ($schemeIds as $schemeItem) {
            DB::statement("
                CREATE TABLE bandhan.bvld_s{$schemeItem}
                PARTITION OF bandhan.validation_lot_details
                FOR VALUES IN ({$schemeItem})
            ");
        }

        DB::statement("
            CREATE TABLE bandhan.bvld_default
            PARTITION OF bandhan.validation_lot_details
            DEFAULT
        ");
    }
