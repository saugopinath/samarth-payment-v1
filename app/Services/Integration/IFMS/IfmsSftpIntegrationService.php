...
class IfmsSftpIntegrationService implements PaymentSBIIntegrationInterface
{
    ...
    public function checkAcknowledge(PaymentLotMaster $lotMaster): array
    {
        try {
            ...
            $lotNo = $lotMaster->lot_no;  
            $fileName = $lotMaster->file_name;
            if ($lotMaster->cur_status != config('payment_lot.status.common.dotdone')) {
                    return [
                        'status' => 3, 
    ...
    public function checkResponse(PaymentLotMaster $lotMaster): string
    {
        try {
           ...
           //dd($fileName1);
            ...
            $remoteFile = Storage::disk("ifms_sftp_{$partyCode}")->get(config('ifms.paths.response') . $fileName);
