...
@if($lot->cur_status == config('payment_lot.status.sbi.not_signed'))
    <button wire:click="signAndPush('{{ $lot->lot_no }}')" class="text-green-600 hover:text-green-900 bg-green-100 px-3 py-1 rounded-md text-xs font-bold transition-colors inline-flex items-center justify-center">
...
@endif
@if($lot->cur_status == config('payment_lot.status.sbi.generated_and_signed'))
    <button wire:click="pushLot('{{ $lot->lot_no }}')" class="text-blue-600 hover:text-blue-900 bg-blue-100 px-3 py-1 rounded-md text-xs font-bold transition-colors inline-flex items-center justify-center">
        ...
        {{ $integrationType == 'api' ? 'Push to API' : 'Push to SBI' }}
    </button>
@endif
@if($lot->cur_status == config('payment_lot.status.sbi.generated_and_pushed'))
    <button wire:click="checkAcknowledge('{{ $lot->lot_no }}')" class="text-blue-600 hover:text-blue-900 bg-blue-100 px-3 py-1 rounded-md text-xs font-bold transition-colors inline-flex items-center justify-center">
        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
        {{ $integrationType == 'api' ? 'Check API Acknowledge' : 'Check Acknowledge' }}
    </button>
@endif
@if($lot->cur_status == config('payment_lot.status.sbi.generated_pushed_response_received'))
    <button wire:click="checkResponse('{{ $lot->lot_no }}')" class="text-purple-600 hover:text-purple-900 bg-purple-100 px-3 py-1 rounded-md text-xs font-bold transition-colors inline-flex items-center justify-center">
        ...
        {{ $integrationType == 'api' ? 'Check API Response' : 'Check Response' }}
    </button>
@endif
@if(in_array($lot->cur_status, [config('payment_lot.status.sbi.not_signed'), config('payment_lot.status.sbi.generated_and_signed')]))
